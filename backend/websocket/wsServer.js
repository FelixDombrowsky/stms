...
export const initWebSocket = (server) => {
  ...
  pollingTimer = setInterval(async () => {
    try {
      ...
      //   temp: 0,
      //   status: 'no_port',
      //   timestamp: '2025-10-25T03:15:41.184Z'
      // },
      ...
      // console.log("Mock Formatted : ", mockFormatted);

      // ส่งไป front-end ทุก 1 วิ
      const payloadStr = JSON.stringify(mockFormatted);

      if (payloadStr !== lastPayloadStr) {
        io.emit("tank_update", mockFormatted);
      ...
      if (now - lastInfluxWrite >= INFLUX_INTERVAL) {
        lastInfluxWrite = now;
        for (const tank of mockFormatted) {
          writeTankData(tank);
        }
      }
    } catch (err) {
